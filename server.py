...
from nlu.data_router import DataRouter
...
def create_app(config, component_builder=None):
	...
	@nlu_app.route('/parse', methods = ['GET'])
	def parse():
	...
	nlu_app.data_router = DataRouter(config, component_builder)
	return nlu_app
		
if __name__ == '__main__':
	arg_parser = create_argparser()
	cmdline_args = {key: val for key, val in list(vars(arg_parser.parse_args()).items()) if val is not None}
	nlu_config = PkrmNLUConfig(cmdline_args.get("config"), os.environ, cmdline_args)
	app = WSGIServer(('0.0.0.0', 5000), create_app(nlu_config))
